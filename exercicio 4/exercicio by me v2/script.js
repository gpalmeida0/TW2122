...
function requestWeatherByCity(){
        const searchValueWeather = $('#weatherSearchID').val();

    $.ajax({
        url: 'https://api.openweathermap.org/data/2.5/weather',
        data: {
            q: searchValueWeather,
        ...
        success: function (response) {
            ...
            $('#weatherContainer').html('');
                    ...
                    const weatherHTMLElement = 'Em ' + response.name + ' estão ' +response.main.temp + ' graus ';
                    $('#weatherContainer').append(weatherHTMLElement);
